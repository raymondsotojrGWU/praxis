...
from sklearn.metrics import confusion_matrix, ConfusionMatrixDisplay, precision_score, recall_score, classification_report, f1_score
...
def submit_update(round_num, weights, num_samples):
    """Submits local model update, returning the parsed JSON response or an error dict."""
...
def main():
    ...
    for round_num in range(client_current_round, FED_ROUNDS):
        ...
        round_start_time = time.time() # New line
        ...
        round_end_time = time.time() # New line
        round_duration_seconds = round_end_time - round_start_time # New line
        print(f"Round {round_num} completed in {round_duration_seconds:.2f} seconds.") # New line
        ...
        if round_num < FED_ROUNDS - 1:
            ...
            try:
                ...
                if status == "Error":
                    ...
                    print(f"Server waiting. Polling for round {next_round_num} weights..."); global_weights, server_round = get_global_weights(expected_round=next_round_num)
